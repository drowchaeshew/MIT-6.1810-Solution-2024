...
static struct vma *valloc(int sz);
static struct vma *vfind(uint64 va);
static int vdump(uint64 va, struct inode *ip, int off, int sz);
...
vload(uint64 va)
{
  ...
  char *mem;
  uint64 addr;
  for (addr = vp->start; addr < vp->end; addr += PGSIZE) {
    if ((mem = kalloc()) == 0) {
      goto bad_kalloc;
    }
...
bad:
  kfree(mem);
bad_kalloc:
  uvmunmap(pgtbl, vp->start, (addr - vp->start) / PGSIZE, 1);
  return -1;
}
